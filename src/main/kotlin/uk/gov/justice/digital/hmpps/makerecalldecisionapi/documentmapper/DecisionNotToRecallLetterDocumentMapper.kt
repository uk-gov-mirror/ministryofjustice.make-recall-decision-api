...
class DecisionNotToRecallLetterDocumentMapper : LetterDocumentMapper() {
  ...
  private fun buildNextAppointmentDetails(recommendation: RecommendationResponse): String? {
    return generateLongDateAndTime(recommendation.nextAppointment?.dateTimeOfAppointment) + "\n"
  }

  private fun buildClosingParagraphOfLetter(recommendation: RecommendationResponse): String? {
    return DecisionNotToRecallLetter.CLOSING_PARAGRAPH.content + recommendation.nextAppointment?.probationPhoneNumber + "\n"
  }
